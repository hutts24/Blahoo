...
{	//defines a simple element structure to hold a pointer to list data
	...
	public:
		template <typename data_pointer> BlahooListElement(const data_pointer *data); 
...
typedef class BlahooList : private BlahooWrapper<Blah_List, BlahooList>
{
	...
	public:
		BlahooList(const char *list_name);
		...
		blah_bool remove_element(const data_pointer *data);
		...
		template <typename data_pointer> 
		void append_element(const data_pointer *data);
			//Appends a new element to the end of the list with given data ptr
	
		template <typename data_pointer> 
		void insert_element(const data_pointer *data);
		...
		template <typename data_pointer> 
		void call_function(blah_list_element_func_1arg function, const data_pointer *arg);
			//Calls function for each element, using data pointer as argument
			//to given function
		...
		template <typename data_pointer> 		
		BlahooListElement *find_element(const data_pointer *data);
		...
		data_pointer *search(blah_list_search_func search_function, const arg_pointer *arg);
