...
//static void junk(Blah_Entity *p, void *pvoid) { std::cout << "JUNK\n"; std::cout.flush(); }

/* Constructors & Destructors */

BlahooEntity::BlahooEntity(char *name, int type) : BlahooWrapper<Blah_Entity, BlahooEntity>(Blah_Entity_new(name, type, 0))
{	//constructs a new plain entity without objects, positioned at origin
	...
	Blah_Entity_set_move_function(base_struct, voidFunctionHook<&BlahooEntity::move>, this); //junk
	//Wrap destroy function in base struct to inherited destructor hook
	Blah_Entity_set_destroy_function(base_struct, destructorHook, this);
} 
...
{	//destroys entity class object 
	...
	std::cout.flush();
	Blah_Entity_set_destroy_function(base_struct, NULL, NULL);
	Blah_Entity_destroy(base_struct); //perform entity object disable on base object
}  
...
BlahooEntityEvent::BlahooEntityEvent(char *name, BlahooEntity *sender) :
 BlahooWrapper<Blah_Entity_Event, BlahooEntityEvent>(Blah_Entity_Event_new(name, sender ? sender->base_struct : NULL, entity_event_function_hook, 0))
{
	...
	base_struct->destroy_function = destructorHook;	//Hook into the destroy function of the base event struct to call C++ hook
}
