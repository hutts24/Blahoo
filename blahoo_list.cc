...
BlahooListElement::BlahooListElement(data_pointer *data) {
...
BlahooList::BlahooList(char *list_name) : BlahooWrapper<Blah_List, BlahooList>(Blah_List_new((char*)list_name))
{	//Creates a new empty list object with given name
}

BlahooList::BlahooList() : BlahooWrapper<Blah_List, BlahooList>(Blah_List_new((char*)""))
{	//Creates a new empty list object without a name (set to empty string)
	//Blah_List_init(base_struct, (char*)"");
}
...
template <typename data_pointer> 
blah_bool BlahooList::remove_element(data_pointer *data) {
...
template <typename data_pointer> 
void BlahooList::call_function(blah_list_element_func_1arg function, data_pointer *arg) {
	//Calls function for each element, using data pointer as argument
	//to given function
	Blah_List_call_with_arg(base_struct, function, (void*)arg);
}
	
void BlahooList::init(char *name) {
	//Sets the name of the list, and all element pointers to NULL
	Blah_List_init(base_struct, (char*)name);
}
	
template <typename data_pointer> 		
BlahooListElement *BlahooList::find_element(data_pointer *data) {
...
data_pointer *BlahooList::search(blah_list_search_func search_function, arg_pointer *arg) {
